	...
	<view style="padding: 20rpx;">
		...
		<navigator url="/pages/address/address" class="box" style="margin: 20rpx 0;">
			<view v-if="addressId">
				...
				<view style="display: flex;">
					...
					<view style="flex: 1; color: #d74582; text-align: right;">30分钟送达</view>
				</view>
			</view>
			<view v-else>
				<uni-icons type="plus" size="18" style="position: relative; top: 4rpx;" color="#d74582"></uni-icons>
				<text style="margin-left: 5rpx; color: #d74582;">请添加收货地址</text>
			</view>
		</navigator>
		...
		<view class="box" style="margin: 20rpx 0;">
			...
			<view v-if="amount.discount">
				<!-- v-if="amount.discount" 防止出现undefined -->
		...
		<view v-if="amount.actual" class="box" style="position: fixed; bottom: 0; width: 100%; left: -40rpx;">
			<!-- position: fixed; bottom: 0; 保证提交按钮一直处于最底部的状态，固定住 -->
			<view style="text-align: right;">
				<text>已优惠</text>
				<text style="color: red;">￥{{ amount.discount }}</text>
				<text style="margin-left: 20rpx;">小计</text>
				...
				<button size="mini" type="primary" style="margin-left: 20rpx; 
					background-color: #c56b90; border-color: #c56b90; position: relative; top: 6rpx;"
		...
		methods: {
			addOrder() { //确认订单页面必须选地址才允许下单
				...
				this.$request.post('/orders/addOrder', form).then(res => {
					...
					if (res.code === '200') {
						...
						setTimeout(() => {//在确认订单页面下单后要记得跳转到订单列表
							uni.switchTab({
			...
			loadCart() { //查询购物车列表
				this.$request.get('/cart/selectAll', {
					...
					console.log(this.cartList, "接收到的购物车信息")
					if (this.cartList.length) {
						this.business = this.cartList[0].business || {} //因为购物车里面所有商家信息都是一样的，所有只用取出第一个数据就行
					}
				})
