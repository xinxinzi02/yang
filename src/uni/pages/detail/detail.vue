	...
	<view style="padding: 20rpx;">
		...
		<view class="box" style="display: flex;">
			...
			<view style="width: 200rpx;height: 200rpx;">
				<image :src="business.avatar" style="width: 100%;height: 100%;border-radius: 10rpx;"></image>
			</view>
		</view>
		<!-- 商品信息结束 -->
		...
		<view style="margin: 20rpx 0;">
			...
			<!-- header -->
			<view style="background-color: #fff; display: flex; border-radius: 10rpx; overflow: hidden; margin-bottom: 10rpx;">
				<!-- overflow: hidden; 表示当元素的内容超出容器的尺寸时，超出的部分将被隐藏，而不会显示在容器之外。 -->
				<view  style="padding: 10rpx;" :class="{'active': current === '全部商品'}" @click="current = '全部商品'"> 
					全部商品
				</view>
				<!-- 当current是全部商品的时候，则全部商品高亮，当点击评价的时候，current重新赋值为评价，则评价高亮，全部商品反之 -->
				<view style="padding: 10rpx 40rpx" :class="{'active': current === '评价'}"
				...
				<scroll-view scroll-y="true" style="height: calc(100vh - 480rpx)">
					...
					<view style="padding: 20rpx;">
						<view style="display: flex; grid-gap: 20rpx; margin-bottom: 20rpx;" v-for="item in goodsList" :key="item.id">
							...
							<view style="width: 24rpx;display: flex;align-items: center;">
								<uni-icons type="plus" size="26" color="#d74582" style="float: right;" @click="addCart(item)"></uni-icons>
							</view>
						</view>										
			...
			<scroll-view scroll-y="true" style="height: calc(100vh - 480rpx);" v-if="current === '评价'">
				<view v-for="item in commentList" :key="item.id" style="border-bottom: 2rpx solid #eee;padding: 20rpx 0;">
					<view style="display: flex;grid-gap: 20rpx;">
						...
						<view style="flex: 1; display: flex; flex-direction: column; justify-content: space-between;">
							<view style="flex: 1; padding-top: 10rpx; font-size: 24rpx;">
								{{ item.content }}
							</view>
							<uni-rate :value="item.star" readonly></uni-rate>
							<!-- readonly是否为只读状态，默认false -->
						</view>
			...
			<scroll-view style="max-height: 70vh;" scroll-y="true">
				<view style="padding: 40rpx 40rpx 140rpx 40rpx;">
					<view style="text-align: right; margin-bottom: 10rpx; color: #999;" v-if="cartList.length">
						<uni-icons style="position: relative; top: 4rpx;" type="trash" size="16" color="#999" @click="deleteAll">
							清空
						</uni-icons>
					</view>
					...
					<view style="margin-top: 40rpx; text-align: right;" v-if="amount.amount">
						总金额<text>¥{{ amount.amount }}</text>
					</view>
					<view style="margin-top: 10rpx; text-align: right;" v-if="amount.discount">
						优惠金额：<text style="color: red;">-¥{{ amount.discount }}</text>
					</view>
					<view style="margin-top: 10rpx; text-align: right;" v-if="amount.actual">
						实付金额：<text style="color: red;font-size: 32rpx;">¥{{ amount.actual }}</text>
					</view>
	...
	export default{
		...
		methods: {
			...
			buttonClick(){
				...
				uni.navigateTo({ //传递参数businessId 点击立即下单的时候，将该商家的ID传到订单页面
			...
			loadCart(){
				...
				this.$request.get('/cart/calc', {
					...
					console.log(this.amount, "获取商品的所有金额信息")
				})
			},
...
<style>
	...
	.category-active {
		...
		color: #9080a4;
		border-left: 5rpx solid #9080a4;
		/* 左边框变成橘色     solid是边框的样式，表示边框是实线的。*/
	}
	
	.mini-btn {
		border: 2rpx solid #d74582;
		padding: 0 4rpx;
		font-size: 22rpx;
		color: #d74582;
		border-radius: 5rpx;
