<template>
	<view>
		<uni-segmented-control :current="current" :values="items" @clickItem="onClickItem" styleType="text"
			activeColor="#9080a4"></uni-segmented-control>
				...
				<view style="display: flex; align-items: baseline; margin-bottom: 10rpx;">
					...
					<view style="display: flex;min-height: 40rpx;">
						...
						<view style="flex: 1; text-align: right;">
							<uni-tag v-if="item.status === '待支付'" text="去支付" size="mini" type="primary"
							...
							<uni-tag v-if="item.status === '待评价'" text="去评价" size="mini" type="royal"
	...
	export default {
		...
		methods: {
			...
			onClickItem(e) {
				...
				if (this.current != e.currentIndex) { //可加可不加
					this.current = e.currentIndex;
					console.log(this.current,'22222')
					this.loadOrders()
				}
			}
		}
