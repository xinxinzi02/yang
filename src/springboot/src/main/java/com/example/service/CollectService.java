...
public class CollectService {
    ...
    /**
     * 查询所有  （在收藏的查询中要包含商家的完整信息）
     */
    public List<Collect> selectAll(Collect collect) {

        List<Collect> collects = collectMapper.selectAll(collect);
        for (Collect c : collects){
          Business business =  businessService.selectById(c.getBusinessId());//拿到商家的信息
            c.setBusiness(business);//放到collect里面的business里面
        }
        return collects;
    }
    ...
    /**
     * 商家点击收藏出现收藏商家效果和取消收藏效果
     */
    public void saveCollect(Collect collect) {
        Collect dbCollect = this.selectByUserIdAndBusinessId(collect.getUserId(), collect.getBusinessId());//先查一遍，看看现在商家处于什么状态，是否被收藏的状态
    ...
    public Collect selectByUserIdAndBusiness(Integer userId, Integer businessId) {
        return collectMapper.selectByUserIdAndBusinessId(userId,businessId);
    }
}