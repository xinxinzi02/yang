...
public class UserService {
    ...
    public void add(User user) {
        ...
        user.setAvatar(http + "1697438073596-avatar.png");//默认头像设置
    ...
    public Account login(Account account) {
        ...
        if (ObjectUtil.isNull(dbUser)) {
            throw new CustomException(ResultCodeEnum.USER_NOT_EXIST_ERROR);//判断数据库中是否存在该用户，不存在的话，直接返回用户不存在
        }
        if (!account.getPassword().equals(dbUser.getPassword())) {   // 比较用户输入密码和数据库密码是否一致
            throw new CustomException(ResultCodeEnum.USER_ACCOUNT_ERROR);
        }
        // 生成token
        String tokenData = dbUser.getId() + "-" + RoleEnum.USER.name();//根据用户id和用户名生成一个token
